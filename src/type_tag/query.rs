...
impl<'s> TypeTagQuery<'s> {
    ...
    pub fn from_type_tag_info(tt: &TypeTagInfo<'s>) -> TypeTagQuery<'s> {
        Self {
            ...
            name: QueryEntry::Value(tt.name.clone()),
            subtypes: QueryEntry::Value(
                tt.subtypes
                    .iter()
                    .map(TypeTagQuery::from_type_tag_info)
                    .collect::<Vec<_>>(),
            ),
        }
    }
    ...
    pub fn test(&self, tt: &TypeTagInfo) -> bool {
...
mod tests {
    ...
    fn test_ttq_display() {
        ...
        let ttq = TypeTagQuery::from_type_tag_info(&tti);
    ...
    fn test_ttq_reduce() {
        let tti = TypeTagInfo::parse("x::y::Z<a::b::C, d::e::F>").unwrap();
        let mut ttq = TypeTagQuery::from_type_tag_info(&tti);
